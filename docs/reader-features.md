...
This page covers a subset of CrossSmudge reader features that go beyond basic page turning. It is not a complete list of every reader setting or action. For a more complete list of features as they were released, see the [releases page](https://github.com/uxjulia/CrossSmudge/releases).

The sections here focus on larger CrossSmudge-specific reader features. Small fixes, implementation details, and features that only arrived from upstream CrossPoint are intentionally left out.
...
CrossSmudge adds a wider range of reader font-sizes, including smaller and larger point sizes for users who want denser pages or much larger text.
...
CrossSmudge supports adjustable reader line spacing from compact to wide spacing.
...
CrossSmudge adds a custom interval picker, so the interval is not limited to the built-in presets. The reader can also remember a different Auto Page Turn interval per book.

Open the reader menu and select **Auto Page Turn** to configure it.

## Time Left

CrossSmudge can show estimated time left in the current chapter or book.
...
CrossSmudge supports EPUB bookmarks from the reader.
...
CrossSmudge supports EPUB text clippings from the reader. Use **Create Clipping**
...
clipping from CrossSmudge removes the saved clipping and highlight from the device
...
CrossSmudge tracks per-book reading stats automatically and aggregates them into global stats.
...
Recent CrossSmudge versions expanded this into a larger stats system, including synced totals, richer X3 stats screens, reading-streak and time charts, editable stat dates, idle-time filtering, reset controls, and all-time stats backup options.
...
CrossSmudge can copy the current EPUB position from one nearby CrossSmudge reader to
...
At 99% book progress, CrossSmudge also shows a popup asking whether to mark the book as finished.
...
CrossSmudge adds reader-focused control options beyond the default button mappings.
